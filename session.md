...
- **The `reps` field is intentionally free text, not numeric.** Coaches enter prescriptions like `"6-8"`, `"AMRAP"`, `"5+"`, `"30s/side"` — all valid programming notation. `kindForColumnId('reps')` returns `undefined` so the input layer doesn't strip non-digit characters. `analytics.ts:parseReps()` extracts the leading number when needed for e1RM math. If you ever feel tempted to "fix" `reps` to be numeric-only, this comment is here to tell you not to.
